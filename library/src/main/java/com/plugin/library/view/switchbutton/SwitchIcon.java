package com.plugin.library.view.switchbutton;
...
public class SwitchIcon extends AppCompatImageView {
    ...
    public SwitchIcon(@NonNull Context context) {
        this(context, null);
    }

    public SwitchIcon(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwitchIcon(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray array = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.SwitchIcon, 0, 0);

        try {
            iconTintColor = array.getColor(R.styleable.SwitchIcon_si_tint_color, Color.BLACK);
            animationDuration = array.getInteger(R.styleable.SwitchIcon_si_animation_duration, DEFAULT_ANIMATION_DURATION);
            disabledStateAlpha = array.getFloat(R.styleable.SwitchIcon_si_disabled_alpha, DEFAULT_DISABLED_ALPHA);
            disabledStateColor = array.getColor(R.styleable.SwitchIcon_si_disabled_color, iconTintColor);
            enabled = array.getBoolean(R.styleable.SwitchIcon_si_disabled_alpha, true);
            noDash = array.getBoolean(R.styleable.SwitchIcon_si_no_dash, false);
        } finally {
            array.recycle();
        }
